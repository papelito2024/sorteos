...
class Access {
  ...
  async user() {
    ...
    if(accessToken==null) throw new AccessError("access denied you must start a session");
    
    const decoded = await tokenManager.verifyToken(accessToken,process.env.JWT_ACCESS_TOKEN_SECRET)
    
    if(!decoded) throw new AccessError(" error invalid broken or expired  access token");
